...
export const loader: LoaderFunction = async ({ request, params }) => {
    ...
    return {
        months: await Promise.all(
            monthRows.map(async (row) => getMonth(row.id))
        ),
        selectedMonth,
        nominations,
        ...
        case "createMonth": {
            ...
            try {
                ...
                const [themeResult] = await pool.execute(
                    `INSERT INTO themes (theme_category_id, name, description)
                     VALUES (?, ?, ?);`,
                ...
                if ('insertId' in themeResult) {
                    ...
                    await pool.execute(
                        `INSERT INTO months (year, month, status, theme_id)
                         VALUES (?, ?, ?, ?);`,
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                    ...
                    <h1 className="text-2xl font-bold flex items-center gap-2 text-zinc-200">
                        ...
                        {["nominating", "jury", "voting"].includes(
                            selectedMonth.status,
                                ...
                                <span
                                    className="inline-flex items-center p-2 px-4 rounded-full text-xs font-medium bg-emerald-400/10 text-emerald-400 ring-1 ring-inset ring-emerald-400/20">
                                    Active Month
                                </span>
                            )}
                    </h1>
            ...
            {selectedMonth && nominations.length > 0 && (
                <section>
                    ...
                    <div
                        className="bg-black/10 backdrop-blur-sm rounded-lg shadow overflow-hidden border border-white/10">
                                    ...
                                    {nominations.map((nomination) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                ...
                                                >
                                                    View Pitches
                                                    ({(nominations.find((n) => n.id === nomination.id)?.pitches || []).length})
                                                </button>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <form method="POST">
                                                    ...
                                                    <button
                                                        type="submit"
                                                        className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${nomination.jurySelected
                                                            ? "bg-blue-500"
                                                            : "bg-zinc-700"
                                                            }`}
                                                    >
                                                        <span
                                                            className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${nomination.jurySelected
                                                                ? "translate-x-5"
                                                                : "translate-x-0"
                                                                }`}
                                                        />
