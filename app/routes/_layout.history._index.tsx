...
export const loader = async () => {
    ...
    const months: Month[] = await Promise.all(monthIds.map(async (row) => {
                                    ...
                                    >
                                        <div className="p-5">
                                            ...
                                            {month.theme && (
                                                <div className="mb-4">
                                                    <span
                                                        className="px-3 py-1 rounded-full text-sm bg-blue-600 text-zinc-100 inline-block whitespace-nowrap overflow-hidden text-ellipsis max-w-full">
                                                        {month.theme.name}
                                                    </span>
                                                </div>
                                            )}
                                                ...
                                                <div className="space-y-4">
                                                    {month.winners.filter(w => !w.short).map(winner => (
                                                        <div key={winner.gameId}
                                                            ...
                                                            <div>
                                                                <div
                                                                    className="text-xs font-medium text-blue-400 mb-1">Long
                                                                    Winner
                                                                </div>
                                                                <div
                                                                    className="text-sm font-medium text-zinc-200">{winner.gameName}</div>
                                                            </div>
                                                        </div>
                                                    ))}
                                                    {month.winners.filter(w => w.short).map(winner => (
                                                        <div key={winner.gameId}
                                                            ...
                                                            <div>
                                                                <div
                                                                    className="text-xs font-medium text-emerald-400 mb-1">Short
                                                                    Winner
                                                                </div>
                                                                <div
                                                                    className="text-sm font-medium text-zinc-200">{winner.gameName}</div>
