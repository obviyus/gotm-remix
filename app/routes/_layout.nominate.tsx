...
import { Form, Link, useActionData, useFetcher, useLoaderData, useNavigation, useSubmit, } from "@remix-run/react";
...
export default function Nominate() {
    ...
    const handleGameSelect = (game: Nomination, existingNomination?: Nomination) => {
        ...
        if (existingNomination) {
            ...
            setEditPitch(existingNomination.pitches.find((p) => p.discordId === userDiscordId)?.pitch || "");
            setIsEditOpen(true);
        } else {
    ...
    const handleEdit = (nomination: Nomination) => {
        ...
        if (fullNomination) {
            ...
            setEditPitch(fullNomination.pitches.find((p) => p.discordId === userDiscordId)?.pitch || "");
            setIsEditOpen(true);
        }
    };
    ...
    const GameSkeleton = () => (
        <div
            className="group relative bg-zinc-900/50 backdrop-blur supports-[backdrop-filter]:bg-zinc-900/20 rounded-xl shadow-lg border border-zinc-800/50 flex h-52 min-w-0">
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            {!hasReachedNominationLimit && (
                <div>
                    ...
                    {isSearching ? (
                        ...
                        <div
                            className="text-center py-12 bg-black/20 backdrop-blur-sm rounded-lg border border-white/10">
            ...
            >
                ...
                <div className="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4">
                    <DialogPanel
                        className="w-full sm:w-[32rem] rounded-t-lg sm:rounded-lg bg-zinc-900 border border-white/10 p-6 shadow-xl">
                        <DialogTitle className="text-lg font-medium leading-6 text-zinc-200 mb-4">
                            Nominate {selectedGame?.gameName} (
                            {selectedGame?.gameYear}
                            )
                        </DialogTitle>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            >
                                <span
                                    className="relative z-10 flex flex-col items-center justify-center gap-1 transition-transform group-hover/btn:scale-105">
                                    Short Game
            ...
            >
                ...
                <div className="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4">
                    <DialogPanel
                        className="w-full sm:w-[32rem] rounded-t-lg sm:rounded-lg bg-zinc-900 border border-white/10 p-6 shadow-xl">
                        <DialogTitle className="text-lg font-medium leading-6 text-zinc-200 mb-4">
                            {editingNomination && editingNomination.pitches.length > 0 ? "Edit" : "Add"} Pitch:{" "}
            ...
            >
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4">
                    <DialogPanel
                        className="w-full max-w-sm rounded-lg bg-zinc-900 border border-white/10 p-6 shadow-xl">
                        <DialogTitle className="text-lg font-medium leading-6 text-zinc-200 mb-4">
                            Delete Nomination
                        </DialogTitle>
